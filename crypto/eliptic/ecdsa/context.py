...
class Curve(object):
    ...
    def __contains__(self, point):
        lh = math.Field.add(point.x.v, self.field_a)
        lh = math.Field.mul(lh, point.x.v)
        lh = math.Field.add(lh, point.y.v)
        lh = math.Field.mul(lh, point.x.v)
        lh = math.Field.add(lh, self.field_b)
        y2 = math.Field.mul(point.y.v, point.y.v)
        lh = math.Field.add(lh, y2)
...
class Domain(object):
    def __init__(self, param_m, nom_k, curve, order, base=None):
        ...
        self.order = order
        self._base = base
        self.modulus = math.Field.comp_modulus(param_m, *self.nom_k)
...
class StandardDomain(Domain):
    ...
    def base(self):
        try:
            ...
            base = math.Point(self.BASE_X, self.BASE_Y)
            self._base = base
